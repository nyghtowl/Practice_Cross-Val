...
weight_path = os.path.join(save_dir, weights_name)
...
print('Model Accuracy = %.2f and Model Loss = %.2f' % (evaluation[1], evaluation[0]))
...
for idx, pred_y in enumerate(predict_gen):
    actual_label = labels['label_names'][np.argmax(y_test[idx])]
    pred_label = labels['label_names'][np.argmax(pred_y)]
    print('Actual Label | Predicted Label = %s | %s' % (actual_label, pred_label))
